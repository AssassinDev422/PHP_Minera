...
'''
        url = 'https://slushpool.com/accounts/profile/json/1697720-7902793772e814218b795a5b2c49dffe'
        ...
        blocks = requests.get('http://api.msunicloud.com:2404/blocks', cookies = {'sid':sid})
        reward_data = {}
        block_data = {}
        for r in rewards.json():
            for index in xrange(0 , len(r['allocations']), 2):
                ra = [r['allocations'][index], r['allocations'][index + 1]]
                if ra[0] == idstr:
                    reward_data[r['blocksid']] = [ra[0], ra[1]]
        for b in blocks.json():
            block_data[b['blockid']] = {'datefound':b['datefound'],'miningduration':b['miningduration'],'datestarted':b['datestarted']}
        url = 'https://slushpool.com/stats/json/1697720-7902793772e814218b795a5b2c49dffe'
        block_data2 = requests.get(url).json()
'''
def main():
    try:
        sid = sys.argv[1]
