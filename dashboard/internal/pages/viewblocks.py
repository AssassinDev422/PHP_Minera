...
def main():
    ...
    html_str = html_str + '<th>Pool Hashrate PH/s</th>'
    html_str = html_str + '<th>Reward ( BTC )</th>'
    html_str = html_str + '<th>Block Value ( BTC )</th>'
    html_str = html_str + '<th>Transactions Value ( BTC )</th>'
    html_str = html_str + '<th>Total Value ( BTC )</th>'
    html_str = html_str + '</tr>'
    html_str = html_str + '</thead>'
    ...
    for i in blocks:
        ...
        html_str = html_str + '<td>' + i['datefound'] + '</td>'
        html_str = html_str + '<td>' + i['miningduration'] + '</td>'
        html_str = html_str + '<td>' + i['poolhashrate'] + '</td>'
        html_str = html_str + '<th>' + str( float(i['reward'])) + 'BTC / $' + str(float( i['reward']) * float(price))  + '</th>'
        html_str = html_str + '<th> 12.5BTC / $' + str(float( 12.5 ) * float(price))  + '</th>'
        html_str = html_str + '<th>' + str( float(i['totalreward']) - 12.5 ) + 'BTC / $' + str((float( i['totalreward']) - 12.5) * float(price))  + '</th>'
        html_str = html_str + '<th>' + str( i['totalreward']) + 'BTC / $' + str(float( i['totalreward']) * float(price))  + '</th>'
        html_str = html_str + '</tr>'
    html_str = html_str +'</tbody>'
